...
let rec expansion exp map threshold =
  ...
  | Knorm.App (f, ys) -> (
      ...
      | Some (args, body) ->
          let env' =
            List.fold_left2 (fun env' (id, t) y -> add id y env') [] args ys
          in
          Alpha.convert body env' )
  | Knorm.Let ((id, typ), def, body) ->
      Knorm.Let
        ((id, typ), expansion def map threshold, expansion body map threshold)
  | Knorm.LetRec ({ name = fun_id, fun_typ; args; body = fun_body }, let_body)
    ->
      (* We decided to implement a version without suppressing the LetRec
       * expression  for now but rather remove it later in the elimination of
       * unnecessary  definitions to avoid problems with a recursive function.
       * *)
      let expanded_fun_body = expansion fun_body map threshold in
      if size fun_body < threshold then
        Knorm.LetRec
          ( { name = (fun_id, fun_typ); args; body = expanded_fun_body },
            expansion let_body
              ((fun_id, (args, expanded_fun_body)) :: map)
              threshold )
      else
        Knorm.LetRec
          ( { name = (fun_id, fun_typ); args; body = expanded_fun_body },
            expansion let_body map threshold )
  | Knorm.LetTuple (vars, defs, body) ->
      Knorm.LetTuple
        (vars, expansion defs map threshold, expansion body map threshold)
  | Knorm.IfEq ((v1, v2), e1, e2) ->
      Knorm.IfEq
        ((v1, v2), expansion e1 map threshold, expansion e2 map threshold)
