...
let rec args_to_asm_pred args regnum =
  match args with
  | h :: r ->
      "ldr r" ^ string_of_int regnum ^ ", [fp, #" ^ h ^ "]\n"
...
let rec exp_to_asm exp =
  ...
  | CallDir (label, args) ->
      ...
      then
        ...
        args_to_asm args ^ "mov fp, sp @ move sp to fp\n" ^ "bl "
...
let rec lfu_to_asm lfu =
  ...
  | fu :: r ->
      ...
      ^ "\n"
      ^ Id.remove_label_undersc fu.name
      ^ ":\n" ^ "str lr, [fp, #4] @ store lr on the stack\n"
      ^ t_to_asm fu.body 0 ^ "ldr pc, [fp, #4] @ load lr (fp + 4) into pc\n"
      ^ lfu_to_asm r
  | [] -> "\n"
