...
let modify_exp exp var_reg =
  ...
  | Add (s1, s2) ->
    (
      match s2 with
      | Var v -> Add (modify_variable s1 var_reg, (Var (modify_variable v var_reg)))
...
let rec modify_t t var_reg =
  match t with
  | Let((variable, typ), exp, t2) -> Let((modify_variable variable var_reg, typ), modify_exp exp var_reg, modify_t t2 var_reg)
  | Ans e -> Ans (modify_exp e var_reg)

let modify_program pg var_reg =
  match pg with
  | Program (lfu, lfl, t) ->
      Program (lfu, lfl, modify_t t var_reg)
